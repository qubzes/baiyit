...
export function CartDrawer() {
  const { items, updateQuantity, removeItem, getTotalPrice, isOpen, closeCart } = useCart()
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            className="fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-xl z-[101] overflow-hidden flex flex-col"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            <div className="p-6 border-b flex items-center justify-between">
              <h2 className="text-2xl font-bold text-primary-navy">Your Cart</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center">
                  ...
                  <Button className="button-accent" onClick={closeCart}>
                    Continue Shopping
                  </Button>
                </div>
              ) : (
