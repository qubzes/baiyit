...
from app.core.middleware import security, get_user
...
async def register(
    ...
    try:
        if await User.get(db, email=user_data.email):
            raise HTTPException(
                status_code=400, detail="An account with this email address already exists. Please use a different email or try to sign in."
            )
...
async def request_otp(
    ...
    try:
        ...
        if not user:
            raise HTTPException(status_code=404, detail="No account found with this email address. Please check the email or register a new account.")
...
async def verify_otp(
    ...
    try:
        ...
        if not user.verify_otp(data.otp):
            raise HTTPException(status_code=401, detail="The verification code you entered is incorrect or has expired. Please try again or request a new code.")
